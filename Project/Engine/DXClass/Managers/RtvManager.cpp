...
void RtvManager::MakeDescriptorHeap() {
	...
	HRESULT hr = GraphicsEngine::Device()->Get()->CreateDescriptorHeap(&heapDesc, IID_PPV_ARGS(&descriptorHeap_));
	assert(SUCCEEDED(hr));
}
...
void RtvManager::Init() {
	...
	// Descriptor生成
	MakeDescriptorHeap();
	descriptorSize_ = GraphicsEngine::Device()->Get()->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
...
void RtvManager::Create(uint32_t index, ID3D12Resource* resource) {
	...
	GraphicsEngine::Device()->Get()->CreateRenderTargetView(resource, &desc_, handle);
}
