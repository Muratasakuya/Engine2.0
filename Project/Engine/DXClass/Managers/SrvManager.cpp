...
void SrvManager::MakeDescriptorHeap() {
	...
	HRESULT hr = GraphicsEngine::Device()->Get()->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&descriptorHeap_));
	assert(SUCCEEDED(hr));
}
...
void SrvManager::Init() {
	...
	// descripptorHeapの作成
	MakeDescriptorHeap();
	descriptorSize_ = GraphicsEngine::Device()->Get()->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
