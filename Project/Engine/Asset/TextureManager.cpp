...
void TextureManager::Init(DXCommon* dxCommon, SrvManager* srvManager) {
...
void TextureManager::Load(const std::string& textureName) {
	...
	texture.resource = CreateTextureResource(GraphicsEngine::Device()->Get(), texture.metadata);
...
ComPtr<ID3D12Resource> TextureManager::UploadTextureData(ID3D12Resource* texture, const DirectX::ScratchImage& mipImages) {
	...
	DirectX::PrepareUpload(GraphicsEngine::Device()->Get(), mipImages.GetImages(), mipImages.GetImageCount(), mipImages.GetMetadata(), subresources);
	uint64_t intermediateSize = GetRequiredIntermediateSize(texture, 0, static_cast<UINT>(subresources.size()));
	ComPtr<ID3D12Resource> intermediateResource = CreateBufferResource(GraphicsEngine::Device()->Get(), intermediateSize);
