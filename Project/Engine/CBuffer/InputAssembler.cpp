...
void InputAssembler::Init(const MeshModelData& meshData) {
	...
	vertices.back().CreateVertexBuffer(GraphicsEngine::Device()->Get(), static_cast<UINT>(meshData.vertices.size()));
	...
	indices.back().CreateIndexBuffer(GraphicsEngine::Device()->Get(), static_cast<UINT>(meshData.indices.size()));
