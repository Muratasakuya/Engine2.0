...
Sprite::Sprite(const std::string& textureName) {
	...
	vertexBuffer_.CreateVertexBuffer(GraphicsEngine::Device()->Get(), kSpriteVertexNum);

	indexBuffer_.data.resize(kSpriteIndexNum);
	indexBuffer_.CreateIndexBuffer(GraphicsEngine::Device()->Get(), kSpriteIndexNum);
...
void Sprite::Draw(BlendMode blendMode) {
	...
	GraphicsEngine::SetRendererPipeline(commandList, Object2D, blendMode);
	...
	D3D12_GPU_DESCRIPTOR_HANDLE gpuHandle = AssetManager::GetTextureGPUHandle(textureName_);
...
void Sprite::SetMetaDataTextureSize() {
	...
	const DirectX::TexMetadata& metadata = AssetManager::GetMetaData(textureName_);
