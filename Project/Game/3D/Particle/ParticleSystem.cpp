...
void ParticleSystem::Draw(const std::string& name, BlendMode blendMode) {
	...
	for (uint32_t index = 0; index < particleGroups_[name].model.data.meshes.size(); ++index) {
		...
		GraphicsEngine::SetRendererPipeline(commandList, NormalParticle, blendMode);
		...
		D3D12_GPU_DESCRIPTOR_HANDLE gpuHandle = AssetManager::GetTextureGPUHandle(particleGroups_[name].model.data.meshes[index].textureName.value());
...
void ParticleSystem::CreateParticle(
	...
	//!! alreadyLoadModel !!//
	particleGroups_[name].model.data = AssetManager::GetModel()->GetModelData(modelName);
