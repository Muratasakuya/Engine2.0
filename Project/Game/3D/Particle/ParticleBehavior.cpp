...
void DispersionBehavior::Update(ParticleData& particle, const Matrix4x4& billboardMatrix) {
	...
	particle.currentTime += GameSystem::GetDeltaTime();
	...
	particle.transform.translate += {
		easedVelocity.x* GameSystem::GetDeltaTime(),
			easedVelocity.y* GameSystem::GetDeltaTime(),
			easedVelocity.z* GameSystem::GetDeltaTime()
	};
...
void InjectionBehavior::Update(ParticleData& particle, const Matrix4x4& billboardMatrix) {
	...
	if (particle.physics.reflectEnable) {
		...
		// 移動処理前の位置
		Vector3 newPosition = particle.transform.translate + easedVelocity * GameSystem::GetDeltaTime();
	...
	Vector3 gravityEffect =
		particle.physics.gravityDirection.value() * particle.physics.gravityStrength.value() * GameSystem::GetDeltaTime();
